...
(function () {
    ...
    (function () {
        ...
        function y(a, b, d, f) {
            ...
            if (f.doCallbacks && a.elementCallbacks) {
                for (var x = a.elementCallbacks, h = 0, o = x.length, w; h < o; ++h)if (w = x[h](b)) {
                    r = w;
                    break;
                }
                if (r)return r
    ...
    (function () {
        function h(a) {
            ...
            var p = {contentDom: 1, key: 1, selectionChange: 1, insertElement: 1, mode: 1}, n;
    ...
    (function () {
        function D(g) {
            for (var i = {}, k = 0; k < g.length; k++)i[g[k]] = !0;
            return i
        }
        ...
        function P() {
            for (var g = 0; g < w.length; g++) {
                var i = w[g], k = null;
                try {
                    k = JSON.parse(r.getItem("autocorrect_" + i))
                } catch (m) {
                }
                null !== k && (CKEDITOR.config["autocorrect_" + i] = k)
            }
        }

        function m(g) {
            var i = new CKEDITOR.dom.walker(g);
            i.evaluator = function (g) {
                return g.type === CKEDITOR.NODE_TEXT && !x(g) || g.type === CKEDITOR.NODE_ELEMENT && "br" == g.getName()
            };
            i.current = g.startContainer;
            this.walker = i;
            this.referenceNode = g.startContainer;
            this.referenceCharacter =
                null;
            this.referenceCharacterOffset = g.startOffset
        }
        ...
        var w = "useReplacementTable recognizeUrlsAsYouType recognizeUrls replaceHyphensAsYouType replaceHyphens formatOrdinalsAsYouType formatOrdinals smartQuotesAsYouType smartQuotes createHorizontalRulesAsYouType createHorizontalRules formatBulletedListsAsYouType formatBulletedLists formatNumberedListsAsYouType formatNumberedLists".split(" "), Q = CKEDITOR.plugins.autocorrect =
        {
            getOption: function (g) {
                return CKEDITOR.config["autocorrect_" + g]
            }, setOption: function (g, i) {
            r.setItem("autocorrect_" + g, i);
            CKEDITOR.config["autocorrect_" + g] = i
        }
        }, x = CKEDITOR.dom.walker.bookmark();
        m.prototype.nextCharacter = function () {
            return null
        };
        m.prototype.previousCharacter = function () {
            for (; 0 === this.referenceCharacterOffset;) {
                this.walker.current = this.referenceNode;
                this.referenceNode = this.walker.previous();
                if (!this.referenceNode || this.referenceNode.type === CKEDITOR.NODE_ELEMENT && "br" == this.referenceNode.getName())return null;
        ...
        CKEDITOR.plugins.add("autocorrect", {
            requires: "menubutton", lang: "en,ru", init: function (g) {
                function i(a) {
                    return Q.getOption(o ? a + "AsYouType" : a)
                }

                function k(a, f) {
                    for (; a && (a.type == CKEDITOR.NODE_ELEMENT && "br" == a.getName() || x(a));)a = f ? a.getPrevious() : a.getNext();
                    return a
                }
                ...
                function B(a) {
                    return "A" <= a && "Z" >= a || "a" <= a && "z" >= a || "а" <= a && "я" >= a || "А" <= a && "Я" >= a || "ё" == a || "Ё" == a || "0" <= a && "9" >= a
                }
                ...
                function n(a) {
                    if (a.startContainer.type == CKEDITOR.NODE_ELEMENT) {
                        var f = a.startContainer.getChild(a.startOffset), b = 0, c = k(f, !0);
                        if (c) {
                            c != f && (b = c.getText().length);
                            for (; c.type == CKEDITOR.NODE_ELEMENT;)c = c.getFirst()
                        } else c = new CKEDITOR.dom.text(""), a.insertNode(c);
                        a.setStart(c, b);
                        a.collapse(!0)
                    }
                }
                ...
                function E(a) {
                    n(a);
                    var f = a.startContainer.getText().substring(a.startOffset - 1, a.startOffset);
                    i("replaceHyphens") && w(f) && F(a, f);
                    var b;
                    if (b = i("smartQuotes"))'"' !== f ? b = !1 : (G(a, R), b = !0);
                    if (!b) {
                        if (b = i("smartQuotes"))"'" !== f ? b = !1 : (G(a, S), b = !0);
                        if (!b) {
                            if (t(f)) {
                                if (b = i("formatBulletedLists"))b:{
                                    b = l(a.startContainer);
                                    if ("p" === b.getName()) {
                                        var c = new CKEDITOR.dom.range(g.editable());
                                        c.selectNodeContents(l(a.startContainer));
                                        var e = new m(c);
                                        e.referenceNode = a.startContainer;
                                        e.referenceCharacterOffset =
                                            a.startOffset;
                                        c = new CKEDITOR.dom.range(g.editable());
                                        c.setEnd(e.referenceNode, e.referenceCharacterOffset);
                                        if (!f || e.previousCharacter()) {
                                            var d = e.previousCharacter();
                                            if (!d || !(d in T)) {
                                                b = !1;
                                                break b
                                            }
                                            c.setStart(e.referenceNode, e.referenceCharacterOffset);
                                            if (e.previousCharacter()) {
                                                b = !1;
                                                break b
                                            }
                                            e = b.getPrevious();
                                            ...
                                            d = a.createBookmark();
                                            c.deleteContents();
                                            !o && e && e.type == CKEDITOR.NODE_ELEMENT && "ul" == e.getName() ? C(b, e) : H([b], "ul", null);
                                            a.moveToBookmark(d);
                                            n(a);
                                            q();
                                            b = !0;
                                ...
                                b || (!i("formatNumberedLists") || !U(a, f)) && i("replaceHyphens") && I(a, f)
                            }
                            (r(f) || t(f)) && J(a, f)
                        }
                    }
                ...
                function V(a, f) {
                    ...
                    b.setStart(a, 0);
                    for (b.collapse(!0); b.startOffset < b.startContainer.getText().length;)b.setStart(b.startContainer, b.startOffset + 1), b.setEnd(b.startContainer, b.startOffset), E(b), f && b.endOffset == b.startContainer.getText().length && K(b);
                ...
                function L(a) {
                    if (a) {
                        var f = a.getText();
                        i("createHorizontalRules") && f.match(W) &&
                        g.document.createElement("hr").replace(a)
                    }
                }

                function X(a) {
                    for (a = a.getNext(); a && x(a);)a = a.getNext();
                    return a
                }
                ...
                function y(a, f) {
                    ...
                    b.evaluator = function (a) {
                        return a.type === CKEDITOR.NODE_ELEMENT && x(a)
                    };
                    for (var c, e = []; c = b.next();)e.push(c);
                    a.deleteContents();
                    for (b = 0; b < e.length; b++)a.insertNode(e[b]);
                    a.insertNode(new CKEDITOR.dom.text(f))
                }

                function F(a, f) {
                    var b = new CKEDITOR.dom.range(g.editable());
                    b.selectNodeContents(l(a.startContainer));
                    var c = new m(b);
                    c.referenceNode = a.startContainer;
                    c.referenceCharacterOffset = a.startOffset;
                    if (!f || c.previousCharacter()) {
                        for (var b = new CKEDITOR.dom.range(g.editable()), e = f, d, h = 1; (d = c.previousCharacter()) && !t(d);)if (d == "--"[h]) {
                            if (1 == h && b.setEnd(c.referenceNode, c.referenceCharacterOffset + 1), h--, 0 > h)break
                        } else e = d, h = 1;
                        if (0 <= h || e && !t(e) && !B(e) && !w(e))return !1;
                        b.setStart(c.referenceNode, c.referenceCharacterOffset);
                        d = c.previousCharacter();
                        if (!d || !B(d) && !('"' == d || "'" == d || "»" == d || "’" == d || "”" == d || "“" == d || ")" == d || "]" == d || "}" == d))return !1;
                        p();
                        d = a.createBookmark();
                        y(b, "—");
                        a.moveToBookmark(d);
                ...
                function I(a, f) {
                    ...
                    if (!f || c.previousCharacter()) {
                        ...
                        if ("-" !== c.previousCharacter())return !1;
                        b.setStart(c.referenceNode, c.referenceCharacterOffset);
                        var e = c.previousCharacter();
                        "-" == e && (b.setStart(c.referenceNode,
                            c.referenceCharacterOffset), e = c.previousCharacter());
                        if (!t(e))return !1;
                        c = u.autocorrect_dash;
                        p();
                        e = a.createBookmark();
                        y(b, c);
                        a.moveToBookmark(e);
                ...
                function J(a, f) {
                    ...
                    if (b = i("recognizeUrls"))a:{
                        if (!r(f)) {
                            ...
                            if (!f || c.previousCharacter()) {
                                b = new CKEDITOR.dom.range(g.editable());
                                b.setEnd(c.referenceNode, c.referenceCharacterOffset);
                                for (var e = !0, d = "", h; (h = c.previousCharacter()) && " " != h && " " != h;)e && h in Y ? b.setEnd(c.referenceNode, c.referenceCharacterOffset) : (d = h + d, e = !1, b.setStart(c.referenceNode, c.referenceCharacterOffset));
                                d = d.match(Z);
                                if (!d) {
                                    b = !1;
                                    break a
                                ...
                                h = d[0];
                                d = "www." === d[1].toLowerCase() ? "http://" + h : h;
                                p();
                                h = a.createBookmark();
                                d = new CKEDITOR.style({element: "a", attributes: {"data-cke-saved-href": d, href: d}});
                    ...
                    if (!b) {
                        if (u.autocorrect_useReplacementTable &&
                            (b = new CKEDITOR.dom.range(g.editable()), b.selectNodeContents(l(a.startContainer)), h = new m(b), h.referenceNode = a.startContainer, h.referenceCharacterOffset = a.startOffset, !f || h.previousCharacter())) {
                            var j;
                            b = new CKEDITOR.dom.range(g.editable());
                            b.setEnd(h.referenceNode, h.referenceCharacterOffset);
                            for (var d = "", k; (k = h.previousCharacter()) && !t(k);)d = k + d, d in M && (j = d, j = M[j], b.setStart(h.referenceNode, h.referenceCharacterOffset));
                            j && (p(), k = a.createBookmark(), y(b, j), a.moveToBookmark(k), n(a), q())
                        }
                        i("formatOrdinals") &&
                        $(a, f);
                        i("replaceHyphens") && F(a, f)
                    }
                }

                function p() {
                    o && (g.fire("saveSnapshot"), N = g.getSelection().getRanges().shift().createBookmark())
                }

                function q() {
                    o && (g.getSelection().selectBookmarks([N]), g.fire("saveSnapshot"))
                }
                ...
                function $(a, f) {
                    ...
                    if (!f || c.previousCharacter()) {
                        ...
                        if (!(e in aa))return !1;
                        b.setStart(c.referenceNode, c.referenceCharacterOffset);
                        for (h = ""; d = c.previousCharacter();)if ("0" <= d && "9" >= d)h = d + h; else {
                            if (B(d))return !1;
                            break
                        }
                        c = h % 100;
                        if (9 < c && 20 > c) {
                            ...
                            if ("rd" !== e)return !1
                        } else if ("th" !== e)return !1;
                        p();
                        e = a.createBookmark();
                        (new CKEDITOR.style({element: "sup"})).applyToRange(b);
                        a.moveToBookmark(e);
                        n(a);
                        q();
                        return !0
                ...
                function U(a, f) {
                    var b = l(a.startContainer);
                    if ("p" === b.getName()) {
                        var c = new CKEDITOR.dom.range(g.editable());
                        c.selectNodeContents(l(a.startContainer));
                        b = new m(c);
                        b.referenceNode = a.startContainer;
                        b.referenceCharacterOffset = a.startOffset;
                        c = new CKEDITOR.dom.range(g.editable());
                        c.setEnd(b.referenceNode, b.referenceCharacterOffset);
                        if (!f || b.previousCharacter()) {
                            if (!(b.previousCharacter()in{".": !0, ")": !0}))return !1;
                            for (var e = "", d; (d = b.previousCharacter()) && " " != d && " " != d;)e = d + e, c.setStart(b.referenceNode,
                                b.referenceCharacterOffset);
                            if (e.match(/^[0-9]+$/))d = "1"; else if (e.match(/^[ivxlcdm]+$/))d = "i"; else if (e.match(/^[IVXLCDM]+$/))d = "I"; else if (e.match(/^[a-z]$/))d = "a"; else if (e.match(/^[A-Z]$/))d = "A"; else return !1;
                            if (b.previousCharacter())return !1;
                            var e = ba(e, d), b = l(a.startContainer), h = b.getPrevious();
                            p();
                            var i = a.createBookmark();
                            c.deleteContents();
                            !o && h && h.type == CKEDITOR.NODE_ELEMENT && "ol" == h.getName() && h.getAttribute("type") == d && h.$.start + h.getChildCount() - 1 == e - 1 ? C(b, h) : H([b], "ol", 1 === e ? {type: d} :
                            {type: d, start: e});
                            a.moveToBookmark(i);
                            n(a);
                            q();
                ...
                function G(a, f) {
                    ...
                    c.previousCharacter();
                    b.setStart(c.referenceNode, c.referenceCharacterOffset);
                    c = (c = c.previousCharacter()) ? 0 > "  –—([{".indexOf(c) : !1;
                    c = f[Number(c)];
                    p();
                ...
                function l(a) {
                    for (; a && (a.type !== CKEDITOR.NODE_ELEMENT || a.getName()in CKEDITOR.dtd.$inline || a.getName()in CKEDITOR.dtd.$empty);)a = a.getParent();
                    return a
                }

                function H(a, f, b) {
                    for (var c = a[a.length - 1].getNext(), f = g.document.createElement(f), e = a[0].getParent(), d; a.length;)d = a.shift(), C(d, f);
                    b && f.setAttributes(b);
                    c ? f.insertBefore(c) : f.appendTo(e)
                }

                function C(a, f) {
                    var b = g.document.createElement("li");
                    a.copyAttributes(b);
                    a.moveChildren(b);
                    a.remove();
                    b.appendTo(f)
                ...
                function j(a) {
                    if (!a)return 0;
                    if ("M" == a.substring(0, 1))return 1E3 + j(a.substring(1));
                    if ("CM" == a.substring(0, 2))return 900 + j(a.substring(2));
                    if ("D" == a.substring(0, 1))return 500 + j(a.substring(1));
                    if ("CD" == a.substring(0, 2))return 400 + j(a.substring(2));
                    if ("C" == a.substring(0, 1))return 100 + j(a.substring(1));
                    if ("XC" == a.substring(0, 2))return 90 + j(a.substring(2));
                    if ("L" == a.substring(0, 1))return 50 + j(a.substring(1));
                    if ("XL" == a.substring(0, 2))return 40 + j(a.substring(2));
                    if ("X" == a.substring(0, 1))return 10 + j(a.substring(1));
                    if ("IX" == a.substring(0, 2))return 9 + j(a.substring(2));
                    if ("V" == a.substring(0, 1))return 5 + j(a.substring(1));
                    if ("IV" == a.substring(0, 2))return 4 + j(a.substring(2));
                    if ("I" == a.substring(0, 1))return 1 + j(a.substring(1))
                }
                ...
                var u = g.config, v = g.lang.autocorrect;
                P();
                g.addCommand("autocorrect", {
                    exec: function (a) {
                        a.fire("saveSnapshot");
                        var f = a.getSelection().getRanges().shift(),
                            b = f.createBookmark();
                        f.collapsed ? (f = new CKEDITOR.dom.range(a.editable()), f.selectNodeContents(a.editable())) : f = f.clone();
                        f = new CKEDITOR.dom.walker(f);
                        a.editable().$.normalize();
                        f.evaluator = function (a) {
                            return a.type === CKEDITOR.NODE_TEXT && !x(a)
                        };
                        for (var c; c = f.next();) {
                            var e = X(c), d = l(c);
                            f.current = V(c, d.isBlockBoundary() && !e || e && e.type === CKEDITOR.NODE_ELEMENT && "br" === e.getName());
                            "p" === d.getName() && !k(e) && L(d)
                        }
                        a.getSelection().selectBookmarks([b]);
                        a.fire("saveSnapshot")
                    }
                });
                var A = g.addCommand("toggleAutocorrect",
                    {
                        preserveState: !0, canUndo: !1, exec: function () {
                        this.setState(z() ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_ON)
                    }
                    }), z = function () {
                    return A.state === CKEDITOR.TRISTATE_ON
                }, s = g.addCommand("autocorrectOptions", new CKEDITOR.dialogCommand("autocorrectOptions"));
                s.canUndo = !1;
                s.readOnly = 1;
                CKEDITOR.dialog.add("autocorrectOptions", this.path + "dialogs/options.js");
                g.addMenuGroup("autocorrectButton");
                s = {};
                s.autoCorrectWhileTyping = {label: v.disable, group: "autocorrectButton", command: "toggleAutocorrect"};
                s.autoCorrectNow =
                {label: v.autocorrectNow, command: "autocorrect", group: "autocorrectButton"};
                s.autocorrectOptions = {label: v.options, command: "autocorrectOptions", group: "autocorrectButton"};
                g.addMenuItems(s);
                g.ui.add("AutoCorrect", CKEDITOR.UI_MENUBUTTON, {
                    label: v.toolbar, modes: {wysiwyg: 1}, toolbar: "spellchecker,20", onRender: function () {
                        A.on("state", function () {
                            this.setState(A.state)
                        }, this)
                    }, onMenu: function () {
                        g.getMenuItem("autoCorrectWhileTyping").label = z() ? v.disable : v.enable;
                        return {
                ...
                g.on("instanceReady", function (a) {
                    a.removeListener();
                    A.setState(u.autocorrect_enabled ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF)
                });
                var o = !1, Y = D(".,?!:()[]{}\"'‘’“”«»<>".split("")), N, M = u.autocorrect_replacementTable, Z = /^(http:|https:|ftp:|mailto:|tel:|skype:|www\.).+$/i, aa = {
                    st: !0,
                    nd: !0,
                    rd: !0,
                    th: !0
                }, W = /^(-{3,}|_{3,})$/, T = D(["*", "+", "•"]), R = u.autocorrect_doubleQuotes, S = u.autocorrect_singleQuotes;
                g.on("key", function (a) {
                    if (!(2228237 !=
                        a.data.keyCode && 13 != a.data.keyCode) && z() && "wysiwyg" === g.mode) {
                        var f = g.getSelection().getRanges().shift(), b = null;
                        13 === a.data.keyCode && (a = l(f.startContainer), "p" === a.getName() && (b = a));
                        setTimeout(function () {
                            o = true;
                            K(f);
                            L(b);
                ...
                g.on("contentDom", function () {
                    g.editable().on("keypress", function (a) {
                        0 !== a.data.$.charCode && z() && setTimeout(function () {
                            o = !0;
                            var a = g.getSelection().getRanges().shift();
                            E(a);
                            o = !1
                        })
                    })
    ...
    CKEDITOR.config.autocorrect_useReplacementTable = !0;
    CKEDITOR.config.autocorrect_recognizeUrlsAsYouType = !0;
    CKEDITOR.config.autocorrect_recognizeUrls = !0;
    CKEDITOR.config.autocorrect_dash = "–";
    CKEDITOR.config.autocorrect_replaceHyphensAsYouType = !0;
    CKEDITOR.config.autocorrect_replaceHyphens = !0;
    CKEDITOR.config.autocorrect_formatOrdinalsAsYouType = !0;
    CKEDITOR.config.autocorrect_formatOrdinals = !0;
    CKEDITOR.config.autocorrect_singleQuotes = "‘’";
    CKEDITOR.config.autocorrect_smartQuotesAsYouType = !0;
    CKEDITOR.config.autocorrect_smartQuotes = !0;
    CKEDITOR.config.autocorrect_doubleQuotes = "“”";
    CKEDITOR.config.autocorrect_createHorizontalRulesAsYouType = !0;
    CKEDITOR.config.autocorrect_createHorizontalRules = !0;
    CKEDITOR.config.autocorrect_formatBulletedListsAsYouType = !0;
    CKEDITOR.config.autocorrect_formatBulletedLists = !0;
    CKEDITOR.config.autocorrect_formatNumberedListsAsYouType = !0;
    CKEDITOR.config.autocorrect_formatNumberedLists = !0;
    (function () {
        function i(a) {
            var b = null != a.config.autosave_SaveKey ? a.config.autosave_SaveKey : "autosave_" + window.location + "_" + a.id, g = null != a.config.autosave_NotOlderThan ? a.config.autosave_NotOlderThan : 1440, d = null != a.config.autosave_saveOnDestroy ? a.config.autosave_saveOnDestroy : !1, c = null != a.config.autosave_saveDetectionSelectors ? a.config.autosave_saveDetectionSelectors : "a[href^='javascript:__doPostBack'][id*='Save'],a[id*='Cancel']";
            ...
            "js/extensions.min.js"), function () {
                ...
                if (localStorage.getItem(b)) {
                    var d = h(b), c = d.data, d = d.saveTime;
                    a.getSnapshot() == c ? localStorage.removeItem(b) : moment(new Date).diff(d, "minutes") > g ? e(b) : (c = a.lang.autosave.loadSavedContent.replace("{0}", moment(d).lang(a.config.language).format(a.lang.autosave.dateFormat)), confirm(c) ? a.openDialog("autosaveDialog") : e(b))
                }
            });
        ...
        function k(a, b) {
            ...
            var d = document.getElementById("cke_autoSaveMessage_" + b.name);
            d && (d.className = "show", setTimeout(function () {
                d.className = "hidden"
            }, 2E3))
        }
        ...
        function l(a, b, c) {
            var c = h(c), d = difflib.stringAsLines(b.getSnapshot()), e = difflib.stringAsLines(c.data), f = (new difflib.SequenceMatcher(d, e)).get_opcodes();
            a.getContentElement("general", "diffContent").getElement().setHtml('<div class="diffContent">' + diffview.buildView({
                ...
                newTextName: b.lang.autosave.autoSavedContent + moment(c.saveTime).lang(b.config.language).format(b.lang.autosave.dateFormat) + "'",
                contextSize: 3,
        ...
        if (function () {
            ...
            CKEDITOR.plugins.add("autosave", {
                version: 0.11, init: function (a) {
                    ...
                    a.on("uiSpace", function (a) {
                        "bottom" == a.data.space && (a.data.html += '<div class="autoSaveMessage" unselectable="on"><div unselectable="on" id="cke_autoSaveMessage_' + a.editor.name + '"class="hidden">' + a.editor.lang.autosave.autoSaveMessage + "</div></div>")
                    }, a, null, 100);
                    a.on("instanceReady", function () {
            ...
            var c = 0, f = !1, j = function (a) {
                ...
                if (f)j(a); else if (a.editor.checkDirty() || a.editor.plugins.bbcode)f = !0, a = a.editor, k(null != a.config.autosave_SaveKey ? a.config.autosave_SaveKey : "autosave_" + window.location + "_" + a.id, a), f = !1
            }
    ...
    (function () {
        ...
        CKEDITOR.plugins.add("allowsave", {
            init: function (a) {
                a.addCommand("save", b).modes = {wysiwyg: 1, source: 1};
                a.ui.addButton && a.ui.addButton("Save", {
                    label: a.lang.allowsave.toolbar,
                    command: "save",
                    toolbar: "document,10"
                })
            }
    ...
    eval(function (d, e, a, c, b, f) {
        ...
        return d
    }("f.r.9('c',{B:'A',y:'c',e:8(g){g.n.9('k',{w:'v k',u:'<7 5=\"\" a=\"\"></7>',i:'j',s:'7(!d){a}',o:8(1){x 1.p=='7'&&1.q('d')},e:8(){0.b('5',0.1.t('5'));0.b('4',0.1.m('4'));0.b('6',0.1.m('h-6'))},2:8(){z 3='';0.1.l('5',0.2.5);3+=0.2.4!=''?'4:'+0.2.4+';':'';3+=0.2.6!=''?'h-6:'+C(0.2.6)+'D;':'';3!=''?0.1.l('a',3):''}});f.i.9('j',0.E+'F/c.G')}});",
        43, 43, "this element data istayl color class size span function add style setData glyphicons glyphicon init CKEDITOR editor font dialog glyphiconsDialog Glyphicons setAttribute getStyle widgets upcast name hasClass plugins allowedContent getAttribute template Insert button return icons var widget requires parseInt px path dialogs js".split(" "), 0, {}));
    var js = {
        ace: "ace.js",
    ...
    CKEDITOR.plugins.add("codeTag", {
        icons: "code", init: function (a) {
            a.addCommand("wrapCode", {
                exec: function (a) {
                    a.insertHtml("<code>" + a.getSelection().getSelectedText() + "</code>")
                }
            });
            a.ui.addButton("Code", {label: "Wrap code", command: "wrapCode", toolbar: "insert"})
        }
    });
    ...
    (function () {
        ...
        CKEDITOR.plugins.add("mathedit", {
            init: function (c) {
                var g = this.path + "icons/mathedit.png";
                c.addCommand("matheditDialog", new CKEDITOR.dialogCommand("matheditDialog", {
                    allowedContent: "img[src,title,class](mathImg)",
                    requiredContent: "img[src,title,class](mathImg)"
                }));
                c.ui.addButton("mathedit", {
                    label: "Insert math",
                    command: "matheditDialog",
                    toolbar: "insert",
                    icon: g
                });
                c.contextMenu && (c.addMenuGroup("Math"), c.addMenuItem("mathedit", {
                    label: "Edit function",
                    icon: g,
                    command: "matheditDialog",
                    group: "Math"
                    ...
                    c.contextMenu.addListener(function (a) {
                        var d = {};
                        a && (a = a.getAscendant("img", !0));
                        if (a && !a.data("cke-realelement") && "mathImg" === a.getAttribute("class"))return d.mathedit = CKEDITOR.TRISTATE_OFF, d
                    }));
                c.on("doubleclick", function (a) {
                    var d = a.data.element;
                    if (d && d.is("img") && d.getAttribute("class") === "mathImg") {
                        a.data.dialog = "matheditDialog";
                        a.cancelBubble = true;
                        a.returnValue = false;
                        a.stop()
                    }
                }, null, null, 1);
                CKEDITOR.dialog.add("matheditDialog", function (a) {
                    var d = "mathedit-latex-input-field-" + f;
                    f = f + 1;
                    return {
                        title: "Add Math",
                        minWidth: 400, minHeight: 200, contents: [{
                            id: "general", label: "Settings", elements: [{
                                type: "html",
                                html: "<span id='" + d + "' style='border: 1px solid gray;padding: 2px;'></span><div style='float: right; padding-top: 10px;'><a href='http://www.codecogs.com/' style='cursor: pointer; font-size: smaller; color:#085585' target='_blank'>CodeCogs</a><br/><a href='http://www.mathquill.com/' style='cursor: pointer; font-size: smaller; color:#085585' target='_blank'>MathQuill</a></div>",
                                setup: function (b) {
                                    var a = jQuery(CKEDITOR.document.getById(d).$);
                                    a.mathquill("editable");
                                    a.mathquill("latex", b.getAttribute("title"))
                                },
                                commit: function (a) {
                                    var c = "http://latex.codecogs.com/gif.latex?", e = "";
                                    (e = jQuery(CKEDITOR.document.getById(d).$).mathquill("latex")) || (e = "empty");
                                    c = c + escape(e);
                            ...
                            b && (b = b.getAscendant("img", true));
                            if (!b || b.getAttribute("class") !== "mathImg" || b.data("cke-realelement")) {
                            ...
                            var b = this.image;
                            this.insertMode && a.insertElement(b);
    ...
    (function () {
        ...
        CKEDITOR.plugins.add("mathjax", {
            requires: "widget,dialog", icons: "mathjax", init: function (b) {
                ...
                b.widgets.add("mathjax", {
                    ...
                    template: '<span class="' +
                    c + '" style="display:inline-block" data-cke-survive=1></span>',
                    parts: {span: "span"},
                    ...
                    init: function () {
                        ...
                        this.once("ready", function () {
                            CKEDITOR.env.ie && a.setAttribute("src",
                ...
                b.on("contentPreview", function (a) {
                    a.data.dataValue = a.data.dataValue.replace(/<\/head>/, '<script src="' + (b.config.mathJaxLib ? CKEDITOR.getUrl(b.config.mathJaxLib) : h) +
                    '"><\/script></head>')
                });
                b.on("paste", function (a) {
        ...
        CKEDITOR.plugins.mathjax.loadingIcon = CKEDITOR.plugins.get("mathjax").path +
        "images/loader.gif";
        CKEDITOR.plugins.mathjax.copyStyles = function (b, c) {
            for (var a = "color font-family font-style font-weight font-variant font-size".split(" "), e = 0; e < a.length; e++) {
                var d = a[e], g = b.getComputedStyle(d);
                g && c.setStyle(d, g)
            }
        };
        CKEDITOR.plugins.mathjax.trim = function (b) {
            var c = b.indexOf("\\(") + 2, a = b.lastIndexOf("\\)");
    ...
    CKEDITOR.config.plugins = 'basicstyles,blockquote,dialogui,dialog,clipboard,panel,floatpanel,menu,contextmenu,resize,button,toolbar,elementspath,enterkey,entities,popup,filebrowser,floatingspace,listblock,richcombo,format,horizontalrule,htmlwriter,wysiwygarea,image,indent,indentlist,fakeobjects,link,list,magicline,maximize,pastetext,pastefromword,removeformat,showborders,sourcearea,specialchar,menubutton,scayt,stylescombo,tab,table,tabletools,undo,wsc,autogrow,autocorrect,autosave,allowsave,glyphicons,pbckcode,codeTag,eqneditor,font,justify,mathedit,lineutils,widget,mathjax';
    ...
    (function () {
        ...
        if (CKEDITOR.env.hidpi) setIcons('bold,0,,italic,24,,strike,48,,subscript,72,,superscript,96,,underline,120,,blockquote,144,,copy-rtl,168,,copy,192,,cut-rtl,216,,cut,240,,paste-rtl,264,,paste,288,,horizontalrule,312,,image,336,,indent-rtl,360,,indent,384,,outdent-rtl,408,,outdent,432,,anchor-rtl,456,,anchor,480,,link,504,,unlink,528,,bulletedlist-rtl,552,,bulletedlist,576,,numberedlist-rtl,600,,numberedlist,624,,maximize,648,,pastetext-rtl,672,,pastetext,696,,pastefromword-rtl,720,,pastefromword,744,,removeformat,768,,source-rtl,792,,source,816,,specialchar,840,,scayt,864,,table,888,,redo-rtl,912,,redo,936,,undo-rtl,960,,undo,984,,spellchecker,1008,,autocorrect--,2064,auto,autocorrect,1056,,autocorrect-,1080,,save,1104,,glyphicons,2256,auto,pbckcode,2304,auto,code,2352,auto,eqneditor,2400,auto,justifyblock,1224,,justifycenter,1248,,justifyleft,1272,,justifyright,1296,,mathedit,2640,auto,mathjax,1344,', 'icons_hidpi.png'); else setIcons('bold,0,auto,italic,33,auto,strike,66,auto,subscript,99,auto,superscript,132,auto,underline,165,auto,blockquote,198,auto,copy-rtl,231,auto,copy,264,auto,cut-rtl,297,auto,cut,330,auto,paste-rtl,363,auto,paste,396,auto,horizontalrule,429,auto,image,462,auto,indent-rtl,495,auto,indent,528,auto,outdent-rtl,561,auto,outdent,594,auto,anchor-rtl,627,auto,anchor,660,auto,link,693,auto,unlink,726,auto,bulletedlist-rtl,759,auto,bulletedlist,792,auto,numberedlist-rtl,825,auto,numberedlist,858,auto,maximize,891,auto,pastetext-rtl,924,auto,pastetext,957,auto,pastefromword-rtl,990,auto,pastefromword,1023,auto,removeformat,1056,auto,source-rtl,1089,auto,source,1122,auto,specialchar,1155,auto,scayt,1188,auto,table,1221,auto,redo-rtl,1254,auto,redo,1287,auto,undo-rtl,1320,auto,undo,1353,auto,spellchecker,1386,auto,autocorrect--,1419,auto,autocorrect,1452,auto,save,1485,auto,glyphicons,1518,auto,pbckcode,1551,auto,code,1584,auto,eqneditor,1617,auto,justifyblock,1650,auto,justifycenter,1683,auto,justifyleft,1716,auto,justifyright,1749,auto,mathedit,1782,auto,mathjax,1815,auto', 'icons.png');
    })();
